...
const RoomManager = _ => {
	...
	const createRoom = _ => dispatch(setCommand('createRoom'));

	const joinRoom = _ => {
		dispatch(resetInputErrors());
		// Give feedback to user, if an error message already exists
		if (!isInputValid) return setTimeout(_ => {
	...
	useEffect(_ => {
		if (nameError) setTimeout(_ => nameInputRef.current.select(), 0);
	}, [nameError]);
	...
	useEffect(_ => {
		...
		if (!nameInput.trim()) {
			setTimeout(_ => nameInputRef.current.select(), 0);
			return;
		}

		setTimeout(_ => roomCodeInputRef.current.select(), 0);
	}, [roomCodeError]);
