...
import { setCommand, setGameValue, setRoomCode } from '../../features/gameSlice';
...
const saveJoinKeyInLocalStorage = ({ joinKey, name, roomCode }) => {
	...
	localStorage.setItem('joinKeys', JSON.stringify({
		...keyObj,
...
const handleRoomCreate = async ({ name, dispatch }) => {
	...
	dispatchRoomJoinData({
		hostId, joinKey, name, roomCode, dispatch,
		id: hostId,
		hostKey: joinKey,
	});

	// Populate socket connection with this player's info
	dispatch(wsCreateRoom({
		hostId, roomCode,
		hostKey: joinKey,
	}));
...
export const createRoom = async ({ nameInput, dispatch }) => {
	if (!nameInput.trim().length)
		return dispatch(setNameError('Enter a name.'));
...
const handleRoomJoin = async (joinParams, dispatch) => {
	...
	if (res.data.joinKey) saveJoinKeyInLocalStorage({
		name, roomCode,
